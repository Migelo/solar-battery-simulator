...
class TestFullPipeline:
    ...
    def test_full_pipeline_runs(self, minimal_production_csv, minimal_consumption_csv):
        ...
        costs = sim.calculate_costs_and_savings(
            peak_price=0.15, off_peak_price=0.10, export_price=0.05
        )
...
class TestEdgeCases:
    ...
    def test_zero_solar(self, minimal_consumption_csv, tmp_path):
        """System works with zero solar production."""
        # Create zero solar production file
        df = pd.DataFrame({"timestamp_id": range(1, 97), "solar_power_kw": [0.0] * 96})

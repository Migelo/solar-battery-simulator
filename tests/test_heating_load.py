...
class TestHeatingLoadDistribution:
    ...
    def test_heating_only_during_specified_hours(
        self, winter_production_csv, winter_consumption_csv
    ):
    ...
    def test_heating_monthly_weights(self, tmp_path):
        ...
        production_df = pd.DataFrame(
            {"timestamp_id": range(1, n_jan + n_mar + 1), "solar_power_kw": [5.0] * (n_jan + n_mar)}
        )
